...
Στη συνέχεια φτιάξτε έναν generator ο οποίος θα παίρνει σαν παράμετρο μία λίστα από strings. Κάθε string θα αποτελείται από 5 χαρακτήρες. Κάθε χαρακτήρας θα είναι είτε: '0' είτε '1' είτε '2':
* Το `0` σημαίνει ότι δεν υπάρχει αυτός ο χαρακτήρας σε καμία θέση
* Το `1` σημαίνει ότι υπάρχει ο χαρακτήρας αλλά σε άλλη θέση.
* Το `2` σημαίνει ότι ο χαρακτήρας είναι σωστός. 
...
O generator `greek_word_gen` κάνει generate ελληνικές λέξεις. 
...
Για παράδειγμα ας υποθέσουμε ότι pattern='τρελα' και το λέξλι επέστρεψε: `01002`. Αυτό σημαίνει ότι:
  * Τα `'Τ'`, `'Ε'`,`'Λ'` δεν υπάρχουν πουθενά στη λέξη που ψάχνουμε
  * Το `'Ρ'` υπάρχει αλλά όχι στη 2η θέση. 
  * Το `'Α'` υπάρχει και είναι στη 5η θέση.   
