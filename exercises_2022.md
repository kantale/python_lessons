...
* Η πρώτη παράμετρος (`C`) θα είναι ένας πίνακα Nx2, 
...
3. Να υπολογίζει τους δείκτες των γραμμώνν (line indexes) οι οποίες υπάρχουν στον C αλλά δεν υπάρχουνν στον `CP` και να το αποθηκεύει σε μία μεταβλητή με το όνομα `non-CP`.

Επιστρέφει τα `CP` και `non-CP`
...
* Έναν ακέραιο `minPTS` 
* Ένα δεκαδικός αριθμό: `eps`
* Έναν αριθμό οποίος θα είναι από το 0 μέχρι και το Ν-1 (έστω `r`)

Η συνάρτηση θα πρέπει να:
1. Αρχικοποιεί ένα άδειο σύνολο S
...
H συνάρτηση θα πρέπει να βρίσκει όλα τα σημεία τα οποία (1) ανήκουν στο non-CP και (2) υπάρχει τουλάχιστον ένα σημείο το οποίο να ανήκει στο `S` και νά έχει απόσταση μικρότερη από `eps`. Η συνάρτηση θα επιστρέφει το σύνολο των σημείων αυτών.
...
Η συνάρτηση θα πρέπει να επιστρέφει τον πίνακα με τα σημεία ου έφτιαξε (δηλαδή 4000 σημεία στο σύνολο)
...
### Άσκηση 80
Φιάξτε μία συνάρτηση η οποια δεν θα παίρνει καμία παράμετρο. Η συνάρτηση θα φτιάχνει μία γραφική παράσταση με ένα 10 x 10 grid με όλες τις γραφικές παραστάσεις της άσκησης 79 για K = range(1,11) και minPTS=range(1,11). 
