...
A = (3,7)
B = (2,5)
print (f(A,B)) # Τυπώνει: 0.5 , (Δηλαδή: (2-3)/(5-7))
...
A = (3,7)
B = (2,5)
print (f(A,B)) # Τυπώνει: -2.0 , (Δηλαδή: -1/((2-3)/(5-7))  )
...
2. Έστω ότι ένα σημείο που ανήκει στην ευθεία ε και ανήκει στον κύκλο έχει συντεταγμένες: x,y. Θα πρέπει να ισχύει:
   ...
   3. Αν αντικαταστήσουμε στην εξίσωση 2 το y από την εξίσωση 1, θα έχουμε:   (x-K)^2 + ((a * x + b)-L)^2 = r^2
